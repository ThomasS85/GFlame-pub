...
if FFF.do_I && ~isfield(myMean,'vel')
  ...
  if strcmpi(FFF.method_I,'source')
    [ myMean.vel ] = distributeSources( [real(myC_L1);imag(myC_L1)] , myM , myR0 , FFF.SPpMM , vel , p , 'source' );
  else
    [ myMean.vel ] = distributePanels( [real(myC_L1),imag(myC_L1)] , myM , FFF.SPpMM , vel , p , 'source' );
  end
...
if doPlot
  ...
  if FFF.do_I
    ...
    [ myC_L1 ] = checkNcorrectLine_geometry( myC_L1 , p );
    if strcmpi(FFF.method_I,'source')
      [ vel , ind_sources ] = distributeSources( [real(myC_L1);imag(myC_L1)] , myM , myR0 , FFF.SPpMM , vel , p , 'source' );
    else
      [ vel ] = distributePanels( [real(myC_L1);imag(myC_L1)] , myM , FFF.SPpMM , vel , p , 'source' , 'write2' , 1 );
    end
...
while t_now<t_max
  ...
  if mod(tt,nth_out)==0
    ...
    if doPlot
      ...
      [~,plts] = draw_FlowField( X_up_mesh , X_down_mesh , vel , x1L_flame_s_old , p , 'meanF' , x1L_flame ,...
