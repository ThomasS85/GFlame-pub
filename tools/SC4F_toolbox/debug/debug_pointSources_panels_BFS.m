...
% myMean.vel.sourceDat.x = [ myMean.vel.sourceDat.x ; L1_to_L2(1i*p.R_i/2,p) ]; 
% myMean.vel.sourceDat.xi = [ myMean.vel.sourceDat.xi ; myMap.xi_x(myMean.vel.sourceDat.x(Nvort0+1:end)) ]; 

% % upstream
Nvort0 = length(myMean.vel.sourceDat.G);
myMean.vel.sourceDat.G = [ myMean.vel.sourceDat.G ; sStrength ]; 
myMean.vel.sourceDat.x = [ myMean.vel.sourceDat.x ; L1_to_L2(-p.R_i*0.7+1i*p.R_i/2,p) ]; 
myMean.vel.sourceDat.xi = [ myMean.vel.sourceDat.xi ; myMap.xi_x(myMean.vel.sourceDat.x(Nvort0+1:end)) ]; 
% 
% % downstream
% Nvort0 = length(myMean.vel.sourceDat.G);
...
draw_FlowField( X_up_mesh , X_down_mesh , myMean.vel , x1L_flame , p , 'scaleU' , 0.001 ,...
  plotSources , ind_sources,'Xi',Xi_all,'plotField','u1','doQuiver',0.001,'fig',myF1,'cLim',[-1 1]*0.4);

% u2
myF1 = figure('Position',[820 50 800 550],'Color','w');hold on;
draw_FlowField( X_up_mesh , X_down_mesh , myMean.vel , x1L_flame , p , 'scaleU' , 0.001 ,...
  plotSources , ind_sources,'Xi',Xi_all,'plotField','u2','doQuiver',0.001,'fig',myF1,'cLim',[-1 1]*0.4);