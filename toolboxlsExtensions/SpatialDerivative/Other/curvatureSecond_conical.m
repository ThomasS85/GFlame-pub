function [ curvature, gradMag ] = curvatureSecond_conical(grid, data)
% curvatureSecond: second order centered difference approx of the curvature for a conical geometry.
%
...
% Ian Mitchell, 6/3/03
%
% Modified to conical by Thomas Steinbacher 13.04.2016: added curvature conical flames
...
% For conical Flames add term (dG/dx_2) / x_2 and compute MEAN CURVATURE c instead of kappa!
%   Only consider flame elements which are away from center axis (x2=0) (otimal would be e.g. 2xMarkstein
%   length)
comCurv = abs(grid.xs{2}) > 0;
curvature(comCurv) = 0.5* ( curvature(comCurv) + first{2}(comCurv) ./ abs(grid.xs{2}(comCurv)) );
