function [ ydot, stepBound, schemeData ] = termConvectionMod(t, y, schemeData)
...
%
% Thomas Steinbacher 11.12.14:
% In case of velocity being a function handle the calculated velocity field
%   is returned as schemeData.velocityField
%
% Thomas Steinbacher 07.01.15:
% In case of velocity being a function handle the velocity field function
%   also returns the schemeData field. Thus the velocity field can be
%   updated from time step to time steps
  ...
  elseif(isa(thisSchemeData.velocity, 'function_handle'))
    ...
    if(iscell(y))
      ...
      if(isfield(thisSchemeData, 'passVLS') && thisSchemeData.passVLS)
        ...
        [velocity,schemeData] = feval(thisSchemeData.velocity, t, vectorData, schemeData);  % EDIT TS: added schemeData as return of function

      else
        % Ignore any vector level set.
        [velocity,schemeData] = feval(thisSchemeData.velocity, t, data, thisSchemeData);  % EDIT TS: added schemeData as return of function
      end
    ...
    % Edit TS: Return also the full velocity field
    schemeData.velocityField = velocity;
